...
const MyAppointment = () => {
  ...
  useEffect(() => {
    if (user) {
      fetch(
        `https://shielded-garden-48042.herokuapp.com/booking?patient=${user.email}`,
        {
          method: "GET",
          headers: {
        ...
        <table className="table w-full ">
          ...
          <tbody>
            {appointment.map((appoint, index) => (
              <tr className="hover" key={appoint._id}>
                <th>{index + 1}</th>
                <td>
                  <div className="flex items-center space-x-3">
                    ...
                    <div>
                      <div className="font-bold">
                        {appoint.patientName || "Normal guy"}
                      </div>
